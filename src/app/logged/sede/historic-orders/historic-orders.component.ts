...
export class HistoricOrdersComponent implements OnInit {
    ...
    displayedColumns: string[] = ['id', 'date', 'dni', 'student', 'state', 'totalPrice', 'actions'];
    dataSourceOrders;
    ...
    onSearch(st: string) {
        ...
        this.filter = this.fb.and(
            ...
            this.fb.or(this.fb.where('student.dni', OPERATORS.CONTAINS, st), this.fb.where('student.full_name', OPERATORS.CONTAINS, st))
        );
